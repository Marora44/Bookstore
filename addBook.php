...
$pID = true;
$fID = "";
if ($_SESSION['id'] < 1){ $pID = false; $publisherName = "admin";}
else {$publisherName = $_SESSION['publisherName']; $fID = $_SESSION['id'];}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fpass) || empty($fID)) $passerr = "&nbsp;&nbsp;Please enter a username/password";
    else {
        $passcheck = "SELECT password from publisher WHERE id = {$_POST['id']}";
        $passres = mysqli_query($dbConnect, $passcheck);
        if (mysqli_num_rows($passres) < 1) $passerr = "Publisher does not exist";
        else {
...
<div class="page">
    ...
    <body>
        ...
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            ...
            Publisher ID: <input type="text" name="id" value="<?php echo $fID; ?>" <?= $disabled ?>><br><br>
